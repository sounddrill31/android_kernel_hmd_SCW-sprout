...
//#define FF_COMPATIBLE_NODE "sil,silead_fp"
#define FF_VDD_GPIO
/*
...
int ff_ctl_init_pins(int *irq_num)
{
	...
	/* Initialize RST pin. */
	gpio = of_get_named_gpio_flags(dev_node, "rst-gpios", 0, &flags);
	if (gpio > 0) {
	...
	gpio = of_get_named_gpio_flags(dev_node, "avdd-gpios", 0, &flags);
	if (gpio > 0) {
	...
	/* Initialize INT pin. */
	gpio = of_get_named_gpio_flags(dev_node, "irq-gpios", 0, &flags);
	if (gpio > 0) {
