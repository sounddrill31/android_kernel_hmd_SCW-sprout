...
static int cdfinger_init_gpio(struct cdfingerfp_data *cdfinger)
{
	...
	CDFINGER_DBG("gpio_is_valid\n");
	if (gpio_is_valid(cdfinger->reset_num)) {
	    CDFINGER_DBG("gpio_request\n");
...
static int cdfinger_power_on(struct cdfingerfp_data *pdata)
{
	...
	CDFINGER_DBG("SUPPORT_ID_NUM\n");
#ifdef POWER_GPIO
	CDFINGER_DBG("0911 gpio_direction_output\n");
	
	CDFINGER_DBG("felix do not set pwr_num, because fp is power on always\n");
	//gpio_direction_output(pdata->pwr_num, 1);
#endif
#ifdef POWER_REGULATOR
	CDFINGER_DBG("regulator_set_voltage\n");
...
{
#ifdef POWER_GPIO
	CDFINGER_DBG("cdfinger_power_off\n");
...
static long cdfinger_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
	...
	CDFINGER_DBG("cdfinger_ioctl\n");
	mutex_lock(&cdfinger->buf_lock);
	switch (cmd) {
		case CDFINGER_INIT_GPIO:
	        CDFINGER_DBG("CDFINGER_INIT_GPIO\n");
		...
		case CDFINGER_RELEASE_DEVICE:
	        CDFINGER_DBG("CDFINGER_RELEASE_DEVICE cdfinger_free_gpio\n");
			cdfinger_free_gpio(cdfinger);
#ifdef SUPPORT_ID_NUM
			cdfinger->chip_id = 0x00;
#endif
	        CDFINGER_DBG("CDFINGER_RELEASE_DEVICE misc_deregister\n");
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	CDFINGER_DBG("start to misc_register\n");
	...
	CDFINGER_DBG("start to mutex_init\n");
...
#endif
	CDFINGER_DBG("input_allocate_device\n");
	...
	CDFINGER_DBG("start to input_allocate\n");
	cdfingerdev->cdfinger_input = input_allocate_device();
	if(input_register_device(cdfingerdev->cdfinger_input))
	{
	    CDFINGER_DBG("input_free_device\n");
