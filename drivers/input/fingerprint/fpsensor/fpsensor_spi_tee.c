...
static int fpsensor_request_named_gpio(fpsensor_data_t *fpsensor_dev, const char *label, int *gpio)
{
    ...
    fpsensor_debug(ERR_LOG, "fpsensor_request_named_gpio label=%s", label);
    if (ret < 0) {
        fpsensor_debug(ERR_LOG, "failed to get '%s'\n", label);
        return ret;
    }
    *gpio = ret;
    fpsensor_debug(ERR_LOG, "devm_gpio_request label=%s", label);
...
static int fpsensor_get_gpio_dts_info(fpsensor_data_t *fpsensor)
{
    ...
    FUNC_ENTRY();
    fpsensor_debug(ERR_LOG, "fpsensor_get_gpio_dts_info() ");
    ...
    fpsensor_debug(ERR_LOG, "FPSENSOR_USE_QCOM_POWER_GPIO");
...
static void fpsensor_disable_irq(fpsensor_data_t *fpsensor_dev)
{
    ...
    fpsensor_debug(INFO_LOG, "now write chipone fingerprint sensor info to /sys/ontim_dev_debug/fpsensor/version\n");
    memset(fp_version,0,sizeof(fp_version));
    snprintf(fp_version, sizeof(fp_version),"%s","ICNF7312BL-A1");
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
    smp_rmb();
#if FPSENSOR_WAKEUP_TYPE == FPSENSOR_WAKEUP_SOURCE
    __pm_wakeup_event(g_ttw_wl, msecs_to_jiffies(1000));
#else
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_INIT:
        ...
        fpsensor_debug(INFO_LOG, " before devm_request_thread_irq\n");
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
    ...
    fpsensor_debug(ERR_LOG, "fpsensor_dev_setup");
    FUNC_ENTRY();

    if (fpsensor_dev_major) {
        fpsensor_debug(ERR_LOG, "MKDEV");
    ...
    fpsensor_debug(ERR_LOG, "device_create\n");
...
static int __init fpsensor_init(void)
{
	...
	//add fingerpirnt info
    fpsensor_debug(INFO_LOG, "----------fpsensor_init-----------");
