...
static const struct vreg_config vreg_conf[] = {
	{ "vdd_ana", 2800000, 2800000, 857000, },
	{ "vcc_spi", 1800000, 1800000, 10, },
	{ "vdd_io", 1800000, 1800000, 10, },
};
	...
	bool enable)
{
    ...
    pr_info("vreg_setup name=%s", name);
...
found:
	//dev_err(dev, "Regulator found===============%s\n", name);
	vreg = fpc1020->vreg[i];
	if (enable) {
	    //dev_err(dev, "regulator enable=true");
		if (!vreg) {
	        //dev_err(dev, "devm_retulator_get()");
		...
		if (regulator_count_voltages(vreg) > 0) {
	        //dev_err(dev, "regulator_set_voltage()");
	    ...
	    //dev_err(dev, "regulator_set_load()");
		rc = regulator_set_load(vreg, vreg_conf[i].ua_load);
		if (rc < 0)
			dev_err(dev, "Unable to set current on %s, %d\n",
					name, rc);

	    //dev_err(dev, "regulator_enable");
	    ...
	    dev_err(dev, "regulator enable=false");
		if (vreg) {
...
static int select_pin_ctl(struct fpc1020_data *fpc1020, const char *name)
{
    ...
    //pr_info("felix select_pin_ctl name=%s", name);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    //pr_info("felix regulator_enable_set");
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
    ...
    //pr_info("felix device_prepare");
	mutex_lock(&fpc1020->lock);
	if (enable && !fpc1020->prepared) {
		fpc1020->prepared = true;
		select_pin_ctl(fpc1020, "fpc1020_reset_reset");

        //pr_info("felix device_prepare vreg_setup vcc_spi");
		rc = vreg_setup(fpc1020, "vcc_spi", true);
		if (rc)
			goto exit;

        //pr_info("felix device_prepare vreg_setup vdd_io");
		rc = vreg_setup(fpc1020, "vdd_io", true);
		if (rc)
			goto exit_1;

        //pr_info("felix device_prepare vreg_setup vdd_ana");
...
static int device_vreg_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	int rc;

    //pr_info("felix device_vreg_prepare");
	mutex_lock(&fpc1020->lock);
	if (enable && !fpc1020->prepared) {
		fpc1020->prepared = true;
		//select_pin_ctl(fpc1020, "fpc1020_reset_reset");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    //pr_info("felix device_prepare_set");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    //pr_info("felix wakeup_enable_set");
	mutex_lock(&fpc1020->lock);
	if (!strncmp(buf, "enable", strlen("enable")))
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    pr_info("felix handle_wakelock_cmd");
	mutex_lock(&fpc1020->lock);
	...
	char *buf)
{
	...
	int irq = gpio_get_value(fpc1020->irq_gpio);
   	...
   	//pr_info("felix irq_get irq=%d", irq);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    pr_info("felix compatible_all_set");
	if(!strncmp(buf, "enable", strlen("enable")) && fpc1020->compatible_enabled != 1){
		...
		rc = fpc1020_request_named_gpio(fpc1020, "fpc,gpio_pwr",
			&fpc1020->pwr_gpio);
		dev_err(dev, "fpc request pwr result = %d\n",rc);
		if (rc)
			goto exit;
	    ...
	    pr_info("felix setlect_pin_ctl(fpc1020, fpc1020_pwr_active)");
		rc = select_pin_ctl(fpc1020, "fpc1020_pwr_active");
		if (rc)
			goto exit;
		...
		snprintf(fp_version, sizeof(fp_version),"%s","FPC1540");
		...
		devm_free_irq(dev, gpio_to_irq(fpc1020->irq_gpio), fpc1020);
        	...
        	memset(fp_version,0,sizeof(fp_version));
        	snprintf(fp_version, sizeof(fp_version),"%s","unknown");
	}
	return count;
exit:
	return -EINVAL;
...
static irqreturn_t fpc1020_irq_handler(int irq, void *handle)
{
	struct fpc1020_data *fpc1020 = handle;
   	...
   	//pr_info("felix fpc1020_irq_handler irq=%d", irq);
	dev_dbg(fpc1020->dev, "%s\n", __func__);
	...
	const char *label, int *gpio)
{
    ...
    //pr_info("felix fpc1020_request_named_gpio *gpio=%d", *gpio);
	if (rc < 0) {
...
static int fpc1020_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(dev->of_node, "fpc,enable-on-boot")) {
		//dev_info(dev, "felix Enabling hardware\n");
		(void)device_vreg_prepare(fpc1020, true);
	}
	...
	if (rc)
		goto exit;
	//pr_info("felix select_pin_ctl(fpc1020, fpc1020_pwr_active)");
	rc = select_pin_ctl(fpc1020, "fpc1020_pwr_active");
	if (rc)
		goto exit;
