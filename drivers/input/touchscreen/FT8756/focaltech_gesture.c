    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return fts_gesture_show(ts_data->dev, attr, buf);
}
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    return fts_gesture_store(ts_data->dev, attr, buf, count);
}
...
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data; // ning.wei++
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_DOUBLECLICK:
        gesture = KEY_WAKEUP;	// ning.wei++ for double click wakeup
        break;
    case GESTURE_O:
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    __set_bit(KEY_WAKEUP, input_dev->keybit);		// ning.wei++ for double click wakeup
...
int fts_gesture_exit(struct fts_ts_data *ts_data)
{
	...
	// ning.wei++
    if (!IS_ERR_OR_NULL(ts_data->tp_gesture_pdev)) {
        platform_device_unregister(ts_data->tp_gesture_pdev);
    }
