...
static struct proc_dir_entry *proc_fts_test;
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_INI_FILE_PATH_RES, file_name);
    if (NULL == pfile) {
    ...
    pos = 0;
    ret = __kernel_write(pfile, data_buf, len, &pos);
    FTS_TEST_ERROR("__kernel_write ret= %d",  ret);
...
static void fts_test_save_result_txt(struct fts_test *tdata)
{
    ...
    FTS_TEST_ERROR("fts_test_save_test_data success");
    ...
    struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    fwname[count - 1] = '\0';
    FTS_TEST_INFO("fwname:%s, length=%d", fwname, count);
...
static const struct file_operations fts_test_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_test_show,
    .write  = fts_test_store,
};
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    proc_fts_test = proc_mkdir("touchscreen", NULL);
    if (!proc_fts_test) {
        FTS_TEST_ERROR("procfs(test) create fail");
        remove_proc_subtree("proc_fts_test", NULL);
	return -ENOMEM;
    } else {
        proc_create_data("fts_test", 0664, proc_fts_test, &fts_test_fops, NULL);
