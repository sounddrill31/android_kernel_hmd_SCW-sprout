...
static long fts_atol(char *nptr)
{
    ...
    /* skip whitespace */
    while ( ispace((int)(unsigned char)*nptr) )
        ++nptr;
...
static int fts_test_read_ini_data(char *config_name, char *config_buf)
{
    ...
    snprintf(fwname, FILE_NAME_LENGTH, "ts/%s", config_name);
    ...
    snprintf(filepath, FILE_NAME_LENGTH, "%s%s",
             FTS_INI_FILE_PATH, config_name);
    FTS_ERROR("open file path= %s", file_path);
    if (NULL == pfile) {
        pfile = filp_open(filepath, O_RDONLY, 0);
    }
    ...
    FTS_TEST_ERROR("request_firmware fwname %s", fwname);
    FTS_TEST_ERROR("request_firmware size %d", fw->size);
    if (ret < 0) {
        FTS_TEST_ERROR("request firmware fail");
        return ret;
    }
