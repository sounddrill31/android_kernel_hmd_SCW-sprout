...
int fts_reset_off(int hdelayms)
{
    FTS_DEBUG("tp reset off");
    gpio_direction_output(fts_data->pdata->reset_gpio, 0);
    if (hdelayms) {
        msleep(hdelayms);
    }
    ...
    do {
        if (fts_read_reg(FTS_REG_FW_VER, &fwver) >= 0) {
            if ((fwver != 0xFF) && (fwver != 0x00)) {
                snprintf(touch_version, sizeof(touch_version),"ft8756 ver %d.0",fwver);
                break;
            }
        }
        cnt++;
