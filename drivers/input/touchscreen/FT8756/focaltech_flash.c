...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "ts/ft8756_fw_"
...
static int fts_read_file(char *file_name, u8 **file_buf)
{
    ...
    snprintf(fwname, FILE_NAME_LENGTH, "ts/%s", file_name);

    if (!upg || !upg->fw) {
        FTS_ERROR("fts_read_file(upg) is faile");
        return -EINVAL;
    }
    ...
    FTS_ERROR("read file name %s", file_name);
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    FTS_ERROR("request_firmware fwname%s", fwname);
    FTS_ERROR("request_firmware  len= %d", fw->size);
    FTS_ERROR("request_firmware  ret=%d", ret);
    if (ret < 0){
        FTS_ERROR("read file fail %d", ret);
        return ret;
    }
    memcpy(*file_buf, (u8 *)fw->data, fw->size);
    ret = fw->size;
    FTS_ERROR("request_firmware  len= %d", ret);
    filp_close(filp, NULL);
    set_fs(old_fs);
