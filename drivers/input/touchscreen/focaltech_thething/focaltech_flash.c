...
{
#if defined(CONFIG_TOUCHSCREEN_FTS_KERNEL_READ_FW)
    int ret = 0;
...
static void fts_ts_register_productinfo(struct fts_ts_data *ts_data)
{
	...
	ret = fts_read_reg(FTS_REG_FW_VER, &ts_data->fw_ver[0]);
	if(ret < 0) {
		FTS_ERROR("FW version read failed\n");
		sprintf(buff_ver,"FW version:Falied");
	} else
		sprintf(buff_ver,"FW version:0x%02x", ts_data->fw_ver[0]);

	ret = fts_read_reg(FTS_REG_VENDOR_ID, &ts_data->module_id);
	if(ret < 0) {
		FTS_ERROR("Module Id read falied.\n");
		sprintf(buff_id, " Module id:Failed");
	}else
		sprintf(buff_id, " Module id:0x%02x", ts_data->module_id);
	//strcat(deviceinfo, buff);
	snprintf(deviceinfo, PAGE_SIZE, "%s %s",
		buff_ver, buff_id);
...
int factory_get_module_id(struct device *dev, char *buf)
{
	...
	err = fts_read_reg(reg_addr, &reg_value);
	if(err < 0){
