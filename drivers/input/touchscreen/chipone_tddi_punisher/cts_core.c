...
int cts_enter_program_mode(struct cts_device *cts_dev)
{
    ...
    i = 0;
    do {
        cts_info("Enter program mode retry %d", i);
#ifdef CONFIG_CTS_I2C_HOST
...
#endif /* CONFIG_CTS_I2C_HOST */
        cts_hw_reg_writel_retry(cts_dev,0x38000,0x83000003,1,0);  
        j = 0; 
        do {     
            udelay(30); 
            cts_info("Read BOOT_MODE retry:%d", j);
            ...
            cts_info("Enter program mode OK and exit");
            return 0;
        } while (j++ <= retries2);    

    } while (i++ < retries1);   

    cts_err("Enter program mode err and exit");
    return -EFAULT; 
}
//add at 2021 05 20  
