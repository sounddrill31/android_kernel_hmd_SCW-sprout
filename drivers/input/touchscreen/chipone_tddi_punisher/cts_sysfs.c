...
static ssize_t cts_gesture_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
...
static ssize_t cts_gesture_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (enable) {
		gesture_mode = 1;
		cts_info("[BOE panel]--gesture_mode---%d--\n", gesture_mode);
...
static DEVICE_ATTR(cts_gesture, S_IRUSR | S_IWUSR, cts_gesture_show, cts_gesture_store);
#endif
...
static ssize_t gesture_en_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *ts_data = chipone_data;

	return cts_gesture_show(ts_data->device, attr, buf);
}
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return cts_gesture_store(ts_data->device, attr, buf, count);
}
...
int cts_sysfs_add_device(struct device *dev)
{
	...
	cts_data->tp_gesture_pdev = platform_device_register_simple("tp_gesture", -1, NULL, 0);
    
	 if (!IS_ERR_OR_NULL(cts_data->tp_gesture_pdev)) {
		 ret = sysfs_create_group(&cts_data->tp_gesture_pdev->dev.kobj, &gesture_en_group);
		 if (ret) {
			 cts_err("gesture common device node create fail");
			 sysfs_remove_group(&cts_data->tp_gesture_pdev->dev.kobj, &gesture_en_group);
		 }
	}
