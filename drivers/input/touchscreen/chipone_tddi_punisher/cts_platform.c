        ...
        int retry, int delay)
{
    ...
    if (pdata->cts_dev->rtdata.program_mode)
    {
        ...
        do {
            ...
            cts_info("cts_plat_spi_read:rx_buf 0x%x,0x%x,0x%x\n", pdata->spi_rx_buf[0],pdata->spi_rx_buf[1],pdata->spi_rx_buf[2]);
        ...
        struct device_node *dev_node)
{
    ...
    if (pdata->irq < 0) {
        cts_err("Parse irq failed %d", pdata->irq);
        return pdata->irq;
    }
...
EXPORT_SYMBOL(active_panel);
int drm_check_dt(struct device_node *np)
{
    int i = 0;
    int count = 0;
    struct device_node *node = NULL;
    struct drm_panel *panel = NULL;
	cts_info("[boe]enter drm_check_dt\n");
    count = of_count_phandle_with_args(np, "panel", NULL);
    if (count <= 0) {
        cts_err("find drm_panel count(%d) fail", count);
        return -ENODEV;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        of_node_put(node);
        if (!IS_ERR(panel)) {
            cts_info("find drm_panel successfully");
            active_panel = panel;
            return 0;
        }
    }

    cts_info("[boe]no find drm_panel");
    return -EPROBE_DEFER;
}
...
#endif
{
...
#ifdef CONFIG_CTS_OF
    {
		...
		ret = drm_check_dt(dev->of_node);
        if (ret != 0) {
            cts_err("parse drm-panel fail");
            return -1;
        }
    }
#endif /* CONFIG_CTS_OF */
