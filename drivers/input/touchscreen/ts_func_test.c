...
#include <linux/platform_device.h> // ning.wei++
...
/*
*	   This function is provided for touchscreen get usb status.
*
* If TP realize this interface---->	int (*ts_usb_plugin_out_status_get)(int plugin),
* CHARGER will use it to notice TP to open or close FH.
*/
int ts_get_usb_plugin_out_status(int plugin)
{
	struct ts_func_test_device *test_device;

	test_device = ts_func_test_device[0];

	if (test_device && test_device->ts_usb_plugin_out_status_get)
		return test_device->ts_usb_plugin_out_status_get(plugin);
	else
		return false;
}
EXPORT_SYMBOL(ts_get_usb_plugin_out_status);
...
static struct platform_device *tp_gesture_pdev;

static ssize_t gesture_en_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct ts_func_test_device *test_device;
	bool enable = false;
...
static ssize_t gesture_en_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t size)
{
	struct ts_func_test_device *test_device;
...
static DEVICE_ATTR(gesture_en, S_IRUGO | S_IWUSR, gesture_en_show,
                   gesture_en_store);

static struct attribute *gesture_en_attrs[] = {
...
static int ts_create_gesture_sysfs(void)
{
    int ret = -1;

    tp_gesture_pdev = platform_device_register_simple("tp_gesture", -1, NULL, 0);
    if (!IS_ERR(tp_gesture_pdev)) {
        ret = sysfs_create_group(&tp_gesture_pdev->dev.kobj, &gesture_en_group);
        if (ret) {
            gen_err("gesture common device node create fail \n");
        }
    }
...
void ts_gen_func_test_init(void)
{
	...
	if (ts_create_gesture_sysfs()) {
		gen_err("create_gesture_sysfs failed!\n");
	}
