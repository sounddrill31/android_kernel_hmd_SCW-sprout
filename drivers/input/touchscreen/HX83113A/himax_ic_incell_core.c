              ...
              int len, uint8_t *data, uint8_t bank)
{
       ...
       w_data = kzalloc(sizeof(uint8_t) * data_len, GFP_KERNEL);
       if (w_data == NULL) {
       ...
       if( g_core_fp.fp_register_write(tmp_addr, data_len, w_data, 0) == NO_ERR )
       {
              ret = true;     
       }
       kfree(w_data);
...
static void himax_update_fw_ver(uint8_t data)
{
	snprintf(touch_version, sizeof(touch_version), "hx83113a ver %d.0",data);
}
...
EXPORT_SYMBOL(himax_mcu_pin_reset);

void himax_mcu_pin_reset_off(void)
{
	I("%s: Now reset the Touch chip.\n", __func__);
	himax_rst_gpio_set(private_ts->rst_gpio, 0);
	usleep_range(RST_LOW_PERIOD_S, RST_LOW_PERIOD_E);
}
EXPORT_SYMBOL(himax_mcu_pin_reset_off);
