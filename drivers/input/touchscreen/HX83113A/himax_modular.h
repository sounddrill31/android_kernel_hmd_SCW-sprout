...
static void himax_add_chip_dt(bool (*detect_fp)(void))
{
	...
	if (idx < 0) {
		...
		entry = (void *)himax_ksym_lookup[idx];
...
void himax_add_chip_dt(bool (*detect_fp)(void))
{
	this_detect_fp = detect_fp;
...
static struct on_flash_operation **kp_on_pflash_op;
