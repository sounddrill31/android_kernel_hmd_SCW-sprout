...
static int himax_get_arraydata_edge(int *RAW)
{
       int temp, i, j, ret=0;
       ...
       ArrayData = kzalloc(sizeof(int) * len, GFP_KERNEL);
       if (ArrayData == NULL) {
              E("%s: allocate ArrayData failed\n", __func__);
              ret = -ENOMEM;
              goto err_get_arraydata_edge;
       }
...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
	...
	// struct file *raw_file = NULL;//all log
	// struct filename *vts_name = NULL;//result file
	// mm_segment_t fs;//
	//loff_t pos = 0;
