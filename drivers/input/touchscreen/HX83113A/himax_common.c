...
extern int himax_hx83113_start(struct himax_chip_entry *entry);
...
static void himax_fb_register(struct work_struct *work)
{
	...
	ts->fb_notif.notifier_call = drm_notifier_callback;
        I("himax_fb_register drm_notifier_callback\n");
	if (active_panel &&
		drm_panel_notifier_register(active_panel, &ts->fb_notif) < 0)
...
static ssize_t gesture_en_store(
    struct device *dev,
...
static int himax_create_gesture_sysfs(struct himax_ts_data *ts)
{
    int ret = -1;

    ts->tp_gesture_pdev = platform_device_register_simple("tp_gesture", -1, NULL, 0);
    if (!IS_ERR(ts->tp_gesture_pdev)) {
        ret = sysfs_create_group(&ts->tp_gesture_pdev->dev.kobj, &gesture_en_group);
        if (ret) {
            E("gesture common device node create fail \n");
        }
    }
...
static void himax_remove_gesture_sysfs(struct himax_ts_data *ts)
{
    if (!IS_ERR_OR_NULL(ts->tp_gesture_pdev)) {
        platform_device_unregister(ts->tp_gesture_pdev);
    }
}
// Add-begin by ning.wei for common gesture device node
...
int himax_chip_common_init(void)
{
	...
	// 	}
		...
		// entry = get_chip_entry_by_index(0);
		entry = kcalloc(1, sizeof(struct himax_chip_detect), GFP_KERNEL);
		himax_hx83113_start(entry);
 ...
 */
    ...
    if (himax_create_gesture_sysfs(ts)) {
        E(" %s: himax_common create_gesture_sysfs failed!\n", __func__);
		goto err_creat_gesture_sysfs_failed;
    }
...
#endif
    himax_remove_gesture_sysfs(ts); // ning.wei++
err_creat_gesture_sysfs_failed:
