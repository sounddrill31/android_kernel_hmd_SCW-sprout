...
*/
struct ft8719_fts_gesture_st {
    u8 gesture_id;
    u8 point_num;
    u16 coordinate_x[FT8719_FTS_GESTURE_POINTS_MAX];
    u16 coordinate_y[FT8719_FTS_GESTURE_POINTS_MAX];
	u8 mode;
	u8 active;
};
...
* Static variables
*****************************************************************************/
static struct ft8719_fts_gesture_st fts_gesture_data;
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct ft8719_fts_gesture_st *gesture = &fts_gesture_data;
    ...
    for (i = 0; i < FT8719_FTS_GESTURE_POINTS_MAX; i++) {
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
	...
	for (index = 0; index < ts_data->gesture_num; index++) {
		if (gesture_id == ts_data->gesture_figure_map[index])
			break;
	}
	if (index >= ts_data->gesture_num) {
		printk("%s couldn't find the matched gesture.\n", __func__);
		return;
	} else {
		if ((ts_data->gesture_state) >> (index + 1) & 1) {
			input_report_key(ts_data->input_dev, ts_data->gesture_func_map[index], 1);
			input_sync(ts_data->input_dev);
			input_report_key(ts_data->input_dev, ts_data->gesture_func_map[index], 0);
			input_sync(ts_data->input_dev);
		} else {
...
/*****************************************************************************
* Name: ft8719_fts_gesture_readdata
...
int ft8719_fts_gesture_readdata(struct ft8719_fts_ts_data *ts_data, u8 *data)
{
    ...
    int i = 0;
    int index = 0;
    u8 buf[FT8719_FTS_GESTURE_DATA_LEN] = { 0 };
    ...
    memcpy(buf, data, FT8719_FTS_GESTURE_DATA_LEN);
    if (buf[0] != ENABLE) {
    ...
    memset(gesture->coordinate_x, 0, FT8719_FTS_GESTURE_POINTS_MAX * sizeof(u16));
    memset(gesture->coordinate_y, 0, FT8719_FTS_GESTURE_POINTS_MAX * sizeof(u16));
...
void ft8719_fts_gesture_recovery(struct ft8719_fts_ts_data *ts_data)
{
	if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        ...
        ft8719_fts_write_reg(0xD1, 0xFF);
        ft8719_fts_write_reg(0xD2, 0xFF);
        ft8719_fts_write_reg(0xD5, 0xFF);
        ft8719_fts_write_reg(0xD6, 0xFF);
        ft8719_fts_write_reg(0xD7, 0xFF);
        ft8719_fts_write_reg(0xD8, 0xFF);
        ft8719_fts_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int ft8719_fts_gesture_suspend(struct ft8719_fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        ft8719_fts_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        ft8719_fts_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int ft8719_fts_gesture_resume(struct ft8719_fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ft8719_fts_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        ft8719_fts_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
...
int ft8719_fts_gesture_init(struct ft8719_fts_ts_data *ts_data)
{
	...
	FTS_FUNC_ENTER();
	for (i = 0; i < ts_data->gesture_num; i++)
		__set_bit(ts_data->gesture_func_map[i], input_dev->keybit);
	...
	memset(&fts_gesture_data, 0, sizeof(struct ft8719_fts_gesture_st));
