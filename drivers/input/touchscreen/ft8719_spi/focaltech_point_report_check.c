...
*****************************************************************************/
#define FT8719_POINT_REPORT_CHECK_WAIT_TIME              200    /* unit:ms */
...
static void fts_prc_func(struct work_struct *work)
{
    struct ft8719_fts_ts_data *ts_data = container_of(work,
                                  struct ft8719_fts_ts_data, prc_work.work);
...
/*****************************************************************************
*  Name: ft8719_fts_prc_queue_work
...
void ft8719_fts_prc_queue_work(struct ft8719_fts_ts_data *ts_data)
{
    ...
    queue_delayed_work(ts_data->ts_workqueue, &ts_data->prc_work,
                       msecs_to_jiffies(FT8719_POINT_REPORT_CHECK_WAIT_TIME));
}

/*****************************************************************************
*  Name: ft8719_fts_point_report_check_init
*  Brief:
*  Input:
*  Output:
...
/*****************************************************************************
*  Name: ft8719_fts_point_report_check_exit
*  Brief:
*  Input:
*  Output:
*  Return:
...
#endif /* FT8719_FTS_POINT_REPORT_CHECK_EN */
