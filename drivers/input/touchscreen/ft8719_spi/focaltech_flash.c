...
#define FT8719_FTS_FLASH_PACKET_LENGTH_SPI_LOW             (4 * 1024 - 4)
#define FT8719_FTS_FLASH_PACKET_LENGTH_SPI                 (32 * 1024 - 16)

#define SYS_TPF_PARTITION               "/dev/block/by-name/tpf"
extern int ft8719_fts_ts_resume(struct device *dev);
/*****************************************************************************
...
*****************************************************************************/
u8 ft8719_fw_file[] = {
#include FT8719_FTS_UPGRADE_FW_FILE
};

u8 ft8719_fw_file2[] = {
#include FT8719_FTS_UPGRADE_FW2_FILE
};

u8 ft8719_fw_file3[] = {
#include FT8719_FTS_UPGRADE_FW3_FILE
};

struct ft8719_upgrade_module ft8719_module_list[] = {
    {FT8719_FTS_MODULE_ID, FT8719_FTS_MODULE_NAME, ft8719_fw_file, sizeof(ft8719_fw_file)},
    {FT8719_FTS_MODULE2_ID, FT8719_FTS_MODULE2_NAME, ft8719_fw_file2, sizeof(ft8719_fw_file2)},
    {FT8719_FTS_MODULE3_ID, FT8719_FTS_MODULE3_NAME, ft8719_fw_file3, sizeof(ft8719_fw_file3)},
};

struct ft8719_upgrade_setting_nf ft8719_upgrade_setting_list[] = {
...
struct ft8719_fts_upgrade *ft8719_fwupgrade;
struct fw_header {
	int panel_id;
	int begin_address;
	int fw_size;
...
struct total_fw_header{
	int force_upgrade_flag;
	int fw_count;
	struct fw_header fw_header_array[4];
};
...
static int fts_check_bootid(void)
{
    ...
    u8 cmd = 0;
    u8 id[2] = { 0 };
    struct ft8719_fts_upgrade *upg = ft8719_fwupgrade;
    ...
    cmd = FTS_CMD_READ_ID;
    ret = ft8719_fts_read(&cmd, 1, id, 2);
    if (ret < 0) {
...
static int fts_enter_into_boot(void)
{
    ...
    for (i = 0; i < FT8719_FTS_UPGRADE_LOOP; i++) {
...
static bool fts_check_fast_download(void)
{
    ...
    ret = ft8719_fts_read_reg(0xdb, &value);
    if (ret < 0) {
        FTS_ERROR("read 0xdb fail");
        goto read_err;
    }

    ret = ft8719_fts_read(cmd, 6, value2, 2);
    if (ret < 0) {
...
static int fts_dpram_write_pe(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    ...
    u32 packet_number = 0;
    u32 packet_len = 0;
    u32 packet_size = FT8719_FTS_FLASH_PACKET_LENGTH_SPI;
    ...
    if ((len < FT8719_FTS_MIN_LEN) || (len > upg->setting_nf->app2_offset)) {
    ...
    if (upg->setting_nf->fd_check) {
        ...
        if (!fd_support)
            packet_size = FT8719_FTS_FLASH_PACKET_LENGTH_SPI_LOW;
    }

    cmd = vmalloc(packet_size + FT8719_FTS_CMD_WRITE_LEN + 1);
    if (NULL == cmd) {
        FTS_ERROR("malloc memory for pram write buffer fail");
        return -ENOMEM;
    }
    memset(cmd, 0, packet_size + FT8719_FTS_CMD_WRITE_LEN + 1);
    ...
    cmd[0] = FT8719_FTS_ROMBOOT_CMD_WRITE;
    for (i = 0; i < packet_number; i++) {
        ...
        for (j = 0; j < packet_len; j++) {
            cmd[FT8719_FTS_CMD_WRITE_LEN + j] = buf[offset + j];
        }

        ret = ft8719_fts_write(&cmd[0], FT8719_FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
static int fts_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    ...
    u8 *cmd = NULL;
    u32 addr = 0;
    u32 baseaddr = wpram ? FT8719_FTS_PRAM_SADDR : FT8719_FTS_DRAM_SADDR;
    u32 offset = 0;
    u32 remainder = 0;
    u32 packet_number = 0;
    u32 packet_len = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        /* set pram address */
        cmd[0] = FT8719_FTS_ROMBOOT_CMD_SET_PRAM_ADDR;
        cmd[1] = BYTE_OFF_16(addr);
        cmd[2] = BYTE_OFF_8(addr);
        cmd[3] = BYTE_OFF_0(addr);
        ret = ft8719_fts_write(&cmd[0], FT8719_FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
        if (ret < 0) {
        ...
        ret = ft8719_fts_write(&cmd[0], 1 + packet_len);
        if (ret < 0) {
            FTS_ERROR("write fw to pram(%d) fail", i);
            goto write_pram_err;
        }
...
static int fts_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    int ret = 0;
    int i = 0;
    u8 cmd[FT8719_FTS_ROMBOOT_CMD_ECC_NEW_LEN] = { 0 };
    u8 value[2] = { 0 };
    struct ft8719_fts_upgrade *upg = ft8719_fwupgrade;
    ...
    cmd[0] = FT8719_FTS_ROMBOOT_CMD_ECC;
    ...
    ret = ft8719_fts_write(cmd, FT8719_FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    cmd[0] = FT8719_FTS_ROMBOOT_CMD_ECC_FINISH;
    for (i = 0; i < FT8719_FTS_ECC_FINISH_TIMEOUT; i++) {
        ret = ft8719_fts_read(cmd, 1, value, 1);
        if (ret < 0) {
    ...
    if (i >= FT8719_FTS_ECC_FINISH_TIMEOUT) {
        FTS_ERROR("wait ecc finish timeout,ecc_finish=%x", value[0]);
        return -EIO;
    }

    /* get ecc value calculate in boot */
    cmd[0] = FT8719_FTS_ROMBOOT_CMD_ECC_READ;
    ret = ft8719_fts_read(cmd, 1, value, 2);
    if (ret < 0) {
...
static int fts_ecc_cal_host(const u8 *data, u32 data_len, u16 *ecc_value)
{
    ...
    u16 i = 0;
    u16 j = 0;
    u16 al2_fcs_coef = FT8719_AL2_FCS_COEF;
...
static int fts_ecc_check(const u8 *buf, u32 len, u32 ecc_saddr)
{
    ...
    int offset = 0;
    u32 packet_size = FT8719_FTS_MAX_LEN_FILE;
    struct ft8719_fts_upgrade *upg = ft8719_fwupgrade;
...
static int fts_pram_write_ecc(const u8 *buf, u32 len)
{
    ...
    /* get pram app length */
    code_len = ((u16)buf[FT8719_FTS_APP_INFO_OFFSET + 0] << 8)
               + buf[FT8719_FTS_APP_INFO_OFFSET + 1];
    code_len_n = ((u16)buf[FT8719_FTS_APP_INFO_OFFSET + 2] << 8)
                 + buf[FT8719_FTS_APP_INFO_OFFSET + 3];
...
static int fts_dram_write_ecc(const u8 *buf, u32 len)
{
    ...
    const_len = ((u16)buf[FT8719_FTS_APP_INFO_OFFSET + 0x8] << 8)
                + buf[FT8719_FTS_APP_INFO_OFFSET + 0x9];
    const_len_n = ((u16)buf[FT8719_FTS_APP_INFO_OFFSET + 0x0A] << 8)
                  + buf[FT8719_FTS_APP_INFO_OFFSET + 0x0B];
    ...
    pram_app_size = ((u32)(((u16)buf[FT8719_FTS_APP_INFO_OFFSET + 0] << 8)
                           + buf[FT8719_FTS_APP_INFO_OFFSET + 1])) * 2;
...
{
    int ret = 0;
    u8 cmd = FT8719_FTS_ROMBOOT_CMD_START_APP;
...
static int ft8719_fts_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
    ...
    snprintf(file_path, FILE_NAME_LENGTH, "%s%s", FT8719_FTS_FW_BIN_FILEPATH, file_name);
...
int ft8719_fts_upgrade_bin(char *fw_name, bool force)
{
    ...
    ret = ft8719_fts_read_file(fw_name, &fw_file_buf);
    if ((ret < 0) || (ret < FT8719_FTS_MIN_LEN)) {
...
int ft8719_fts_enter_test_environment(bool test_state)
{
    ...
    msleep(50);
    ret = ft8719_fts_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &detach_flag);
...
int ft8719_fts_fw_recovery(void)
{
    ...
    ret = ft8719_fts_read_reg(0xD0, &boot_state);
    if (ret < 0) {
    ...
    ret = ft8719_fts_fw_resume();
    if (ret < 0) {
        FTS_ERROR("ft8719_fts_fw_resume fail");
        return ret;
    }
    msleep(10);
    ret = ft8719_fts_read_reg(FTS_REG_CHIP_ID, &chip_id);
    FTS_INFO("read chip id:0x%02x", chip_id);

    ft8719_fts_tp_state_recovery(upg->ts_data);

    FTS_INFO("boot recovery pass");
    return ret;
}

static void fts_ts_register_productinfo(struct ft8719_fts_ts_data *ts_data)
{
	...
	ret = ft8719_fts_read_reg(FTS_REG_FW_VER, &ts_data->fw_ver[0]);
	if(ret < 0) {
		FTS_ERROR("FW version read failed\n");
		sprintf(deviceinfo,"FW version:Falied");
	} else
		sprintf(deviceinfo,"FW version:0x%02x", ts_data->fw_ver[0]);

	ret = ft8719_fts_read_reg(FTS_REG_VENDOR_ID, &ts_data->module_id);
	if(ret < 0) {
		FTS_ERROR("Module Id read falied.\n");
		sprintf(buff, " Module id:Failed");
	}else
		sprintf(buff, " Module id:0x%02x", ts_data->module_id);
...
#if 0
int ft8719_factory_get_module_id(struct device *dev, char *buf)
{
	struct ft8719_fts_ts_data *data = dev_get_drvdata(dev);
	u8 reg_value, reg_addr = FTS_REG_VENDOR_ID;
	int err = 0;

	if(data == NULL){
		FTS_ERROR("ft8719_fts_ts_data is NULL.\n");
		return snprintf(buf, PAGE_SIZE, "Failed\n");
	}

	err = ft8719_fts_read_reg(reg_addr, &reg_value);
	if(err < 0){
		FTS_ERROR("Module Id read failed.\n");
		return snprintf(buf, PAGE_SIZE, "Failed\n");
	}
	data->module_id = reg_value;
	printk("%s focal module id:%x.\n", __func__, data->module_id);
...
/*static int __init lcd_name_get(char *str)
{
	...
	strncpy(buf, str, 2);
	FTS_INFO("before tp vendor id from uboot: str = %s, buf = %s, default tp_module_id = %d\n", str, buf,
		 tp_module_id);
	if (strncmp(buf, "e0", strlen("e0")) == 0) {
		tp_module_id = 0xE0;
		FTS_INFO("tp_module_id = 0xe0\n");
	}else if (strncmp(buf, "f0", strlen("f0")) == 0) {
		tp_module_id = 0xF0;
		FTS_INFO("tp_module_id = 0xf0\n");
	}else if(strncmp(buf, "87", strlen("87")) == 0){
		tp_module_id = 0xE1;
		FTS_INFO("To separate the module of FT8719_huaxian(550TB3) from FT8006S_AA_huaxian(FT8006S), change the tp_module_id from 0xe0 to 0xe1\n");
	}
...
int ft8719_factory_get_module_id(struct device *dev, char *buf)
{
	...
	data->module_id = tp_module_id;
	printk("%s focal module id:%x,tp_module_id = %x.\n", __func__, data->module_id,tp_module_id);

	return snprintf(buf, PAGE_SIZE, "0x%02X\n", data->module_id);
}
static int fts_fwupg_get_module_info(struct ft8719_fts_upgrade *upg)
{
    int i = 0;
    struct ft8719_upgrade_module *info = &ft8719_module_list[0];
...
static int fts_get_fw_file_via_request_firmware(struct ft8719_fts_upgrade *upg)
{
    int ret = 0;
    const struct firmware *fw = NULL;
    u8 *tmpbuf = NULL;
...
static int fts_get_fw_file_via_i(struct ft8719_fts_upgrade *upg)
{
 ...
 *         MODULE_ID/FT8719_FTS_MODULE_NAME;
 ...
 *         =1, and FT8719_FTS_MODULE_NAME; fw file name will be composed of "focalt-
 ...
 *****************************************************************************/
static int fts_fwupg_get_fw_file(struct ft8719_fts_upgrade *upg)
{
    int ret = 0;
    ...
    if (upg->fw_length < FT8719_FTS_MIN_LEN) {
...
static void fts_fwupg_delay_work(struct work_struct *work)
{
    /*int ret = 0;
    u8 chip_id = 0;
    struct ft8719_fts_upgrade *upg = ft8719_fwupgrade;
	...
	int i = 0;
	struct total_fw_header total_header;
#if !FT8719_FTS_AUTO_UPGRADE_EN
    FTS_INFO("FT8719_FTS_AUTO_UPGRADE_EN is disabled, not upgrade when power on");
    return ;
#endif

    FTS_INFO("fts_fwupg_delay_work enter.");
	ret = his_read_file(SYS_TPF_PARTITION, 0, &total_header, sizeof(total_header));
	if(total_header.force_upgrade_flag==0 || ret<0){
		FTS_ERROR("force_upgrade_flag=%d, read ret=%d", total_header.force_upgrade_flag, ret);
		return;
	}

	for(i = 0; i < total_header.fw_count; i++) {
		FTS_INFO("read header%d: panel_id=%d, begin_address=%d, fw_size=%d", i+1, 
			total_header.fw_header_array[i].panel_id, 
			total_header.fw_header_array[i].begin_address, 
			total_header.fw_header_array[i].fw_size);
		if(total_header.fw_header_array[i].panel_id == upg->module_id)
			break;
	}

	if(total_header.fw_header_array[i].panel_id == upg->module_id) {
		root_state = kzalloc(total_header.fw_header_array[i].fw_size, GFP_KERNEL);
		if (root_state == NULL) {
			FTS_ERROR("%s: kzalloc fw buffer = NULL", __func__);
			return;
		}

		ret = his_read_file(SYS_TPF_PARTITION, 
			total_header.fw_header_array[i].begin_address, 
			root_state, 
			total_header.fw_header_array[i].fw_size);
		if (ret < 0) {
			FTS_ERROR("his_read_file err, ret=%d", ret);
			kfree(root_state);
			return;
		}
	} else
	{
		FTS_ERROR("module id is error.\n");
		return;
	}


	ret = fts_fw_download(root_state, total_header.fw_header_array[i].fw_size, true);
	if (ret < 0) {
	...
	fts_ts_register_productinfo(upg->ts_data);

	upg->ts_data->suspended = true;
	ft8719_fts_ts_resume(upg->ts_data->dev);*/
}
...
//#if !FT8719_FTS_AUTO_UPGRADE_EN
	...
	upg->ts_data->suspended = true;
	ft8719_fts_ts_resume(upg->ts_data->dev);
	INIT_DELAYED_WORK(&upg->ts_data->fwupg_delay_work, fts_fwupg_delay_work);
	queue_delayed_work(upg->ts_data->ts_workqueue, &upg->ts_data->fwupg_delay_work,
					usecs_to_jiffies(15000000));
}

int ft8719_factory_get_fs_fw_version(struct device *dev, char *buf)
{
	int ret = 0;
	int fs_fw_version = 0;
	struct ft8719_fts_upgrade *upg = ft8719_fwupgrade;
	...
	msleep(50);
	fs_fw_version = upg->fw[270];
	printk("%s: fs_fw_version = 0x%02x\n", __func__, fs_fw_version);
out:
...
{
    int i = 0;
    struct ft8719_upgrade_setting_nf *setting = &ft8719_upgrade_setting_list[0];
    int setting_count =
    ...
    ft8719_fwupgrade = (struct ft8719_fts_upgrade *)kzalloc(sizeof(*ft8719_fwupgrade), GFP_KERNEL);
