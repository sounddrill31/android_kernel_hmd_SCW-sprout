...
*****************************************************************************/
enum {
    FT8719_RWREG_OP_READ = 0,
    FT8719_RWREG_OP_WRITE = 1,
};

/*****************************************************************************
* Static variables
*****************************************************************************/
static struct ft8719_rwreg_operation_t {
    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    struct ft8719_ftxxxx_proc *proc = &ts_data->proc;
    ...
    case PROC_WRITE_REGISTER:
        ret = ft8719_fts_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        writelen = buflen - 1;
        if (writelen >= FT8719_FTX_MAX_COMMMAND_LENGTH) {
    ...
    case PROC_WRITE_DATA:
        writelen = buflen - 1;
        ret = ft8719_fts_write(writebuf + 1, writelen);
        if (ret < 0) {
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            ft8719_fts_enter_test_environment(0);
        } else {
            ft8719_fts_enter_test_environment(1);
        }
        break;
    ...
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
...
#if FT8719_FTS_ESDCHECK_EN
    ft8719_fts_esdcheck_proc_busy(1);
#endif

    switch (proc->opmode) {
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = ft8719_fts_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        ret = ft8719_fts_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
#if FT8719_FTS_ESDCHECK_EN
    ft8719_fts_esdcheck_proc_busy(0);
#endif
...
int ft8719_fts_create_apk_debug_channel(struct ft8719_fts_ts_data *ts_data)
{
    struct ft8719_ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
void ft8719_fts_release_apk_debug_channel(struct ft8719_fts_ts_data *ts_data)
{
    struct ft8719_ftxxxx_proc *proc = &ts_data->proc;
...
{
    ssize_t count = 0;
    struct irq_desc *desc = irq_to_desc(ft8719_fts_data->irq);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        ...
        ft8719_fts_data->fw_is_running = false;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[EX-FUN]set to fw mode");
        ft8719_fts_data->fw_is_running = true;
    }
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&input_dev->mutex);
    if (true == ft8719_fts_data->fw_is_running) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8719_fts_esdcheck_proc_busy(1);
#endif
    ft8719_fts_read_reg(FTS_REG_FW_VER, &fwver);
#if FT8719_FTS_ESDCHECK_EN
    ft8719_fts_esdcheck_proc_busy(0);
#endif
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (rw_op.len < 0) {
        ...
        if (FT8719_RWREG_OP_READ == rw_op.type) {
            if (rw_op.res == 0) {
...
static int fts_parse_buf(const char *buf, size_t cmd_len)
{
    ...
    if (buf[0] == '1') {
        rw_op.len = length;
        rw_op.type = FT8719_RWREG_OP_READ;
        ...
        rw_op.type = FT8719_RWREG_OP_WRITE;
        rw_op.len = length + 1;
    }
    ...
    if (rw_op.len > 0) {
        ...
        if (FT8719_RWREG_OP_WRITE == rw_op.type) {
    ...
    if (rw_op.len < 0) {
        ...
        if (FT8719_RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = ft8719_fts_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = ft8719_fts_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = ft8719_fts_write_reg(reg, val);
            } else {
                rw_op.res = ft8719_fts_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
/* ft8719_fts_upgrade_bin interface */
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    ft8719_fts_upgrade_bin(fwname, 0);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    ft8719_fts_upgrade_bin(fwname, 1);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < ft8719_fts_data->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", ft8719_fts_data->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    FTS_DEBUG("log level:%d->%d", ft8719_fts_data->log_level, value);
    ft8719_fts_data->log_level = value;
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
/*  upgrade from fw bin file   example:echo "*.bin" > ft8719_fts_upgrade_bin */
