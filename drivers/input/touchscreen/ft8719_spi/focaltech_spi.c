...
{
    int ret = 0;
    struct spi_device *spi = ft8719_fts_data->spi;
...
static int fts_wait_idle(void)
{
    ...
    u8 *txbuf = ft8719_fts_data->bus_tx_buf;
    u8 *rxbuf = ft8719_fts_data->bus_rx_buf;
    u32 txlen = 0;

    if (!ft8719_fts_data->fw_is_running)
        idle_status = 0x00;
...
static int fts_cmdpkg_wirte(u8 ctrl, u8 *cmd, u32 cmdlen)
{
    ...
    if ((!cmd) || (!cmdlen)
        || (cmdlen > (FT8719_FTX_MAX_COMMMAND_LENGTH - SPI_HEADER_LENGTH))) {
...
static int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    u8 tmpcmd[FT8719_FTX_MAX_COMMMAND_LENGTH] = { 0 };
...
{
    return ft8719_fts_read(&addr, 1, value, 1);
}
