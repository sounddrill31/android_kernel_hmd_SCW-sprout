...
static bool enable_debug_log = 1;
...
// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_reset = devm_gpiod_get_index(dev, "reset",0 , GPIOD_OUT_HIGH);
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
...
// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_pidle = devm_gpiod_get_index(dev, "pidle",0 , GPIOD_IN);
	...
	st21nfc_dev->gpiod_clkreq = devm_gpiod_get_index(dev, "clkreq",0 , GPIOD_IN);
