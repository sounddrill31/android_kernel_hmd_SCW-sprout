...
#define AW8624_DRIVER_VERSION "v1.3.10"
...
static int aw8624_haptic_play_go(struct aw8624 *aw8624, bool flag)
{
	...
	if (!flag) {
		ktime_get_real_ts64(&aw8624->current_time);
	...
	if (flag == true) {
		...
		ktime_get_real_ts64(&aw8624->pre_enter_time);
	} else {
		aw8624_i2c_write_bits(aw8624, AW8624_REG_GO,
...
static int aw8624_haptic_set_f0_preset(struct aw8624 *aw8624)
{
	...
	pr_info("%s, f0_reg=0x%x, f0_pre=0x%x, f0_coeff=0x%x\n", __func__, f0_reg,
			aw8624->f0_pre, aw8624_dts_data.aw8624_f0_coeff);
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		aw8624->f0_calib_state = true;
		pr_err("%s final f0_calib_data=%d[0x%02x]\n", __func__,
				aw8624->f0_calib_data, f0_cali_lra);
		/* update cali step */
		aw8624_i2c_write(aw8624,
				AW8624_REG_TRIM_LRA,
...
static ssize_t aw8624_f0_calib_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "%d:%s\n",
			aw8624->f0_calib_data, aw8624->f0_calib_state ? "done" : "undo");

	return len;
}
		
static ssize_t aw8624_f0_calib_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	rc = kstrtouint(buf, 0, &val);
	if (rc < 0) {
		pr_err("%s abnormal data!\n", __func__);
		return rc;
	}

	aw8624->f0_calib_data = val;
	aw8624->f0_calib_state = true;
	pr_err("%s calib data = %d\n", __func__, aw8624->f0_calib_data);
...
static DEVICE_ATTR(f0_calib, 0664, aw8624_f0_calib_show, aw8624_f0_calib_store);
...
static void aw8624_interrupt_setup(struct aw8624 *aw8624)
{
	...
	//Daniel 20210225 modify
	aw8624_i2c_write_bits(aw8624,
	AW8624_REG_ANACTRL,
	AW8624_BIT_ANACTRL_LRA_SRC_MASK,
	AW8624_BIT_ANACTRL_LRA_SRC_REG);//Daniel 20210225 modify
}
...
aw8624_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	aw8624_haptic_set_f0_preset(aw8624);//Daniel 20210226 modify
