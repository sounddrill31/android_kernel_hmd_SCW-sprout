...
static int aw8695_haptic_play_go(struct aw8695 *aw8695, bool flag)
{
	...
	if (flag) {
		...
		getnstimeofday(&aw8695->pre_enter_time);
	} else {
		aw8695_i2c_write_bits(aw8695, AW8695_REG_GO,
				      AW8695_BIT_GO_MASK,
				      AW8695_BIT_GO_DISABLE);
		getnstimeofday(&aw8695->current_time);
		...
		if (aw8695->interval_us < 2000) {
			pr_info("aw8695->interval_us=%d < 2000\n",
				aw8695->interval_us);
			mdelay(2);
		}
	}
...
static int aw8695_rtp_osc_calibration(struct aw8695 *aw8695)
{
	...
	while (1) {
		...
		aw8695->microsecond =
		   ...
		   ( (aw8695->end.tv_nsec - aw8695->start.tv_nsec) / 1000);
...
static int aw8695_haptic_ram_config(struct aw8695 *aw8695){
	...
	if((aw8695->duration > 0) && (aw8695->duration < 20)){
		wavseq = 3;
		wavloop = 0;
	} else if((aw8695->duration > 20) && (aw8695->duration <= 30)){
		wavseq = 2;
		wavloop = 0;
	} else if((aw8695->duration > 30) && (aw8695->duration <= 60)){
		wavseq = 1;
		wavloop = 0;
	 } else {
		wavseq = 4;
...
static void aw8695_vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw8695->state) {
		if (aw8695->activate_mode == AW8695_HAPTIC_ACTIVATE_RAM_MODE) {
			...
			if(aw8695->duration > 60){//long ram vib
		...
		/* run ms timer */
		if(aw8695->duration > 60){//long vib
