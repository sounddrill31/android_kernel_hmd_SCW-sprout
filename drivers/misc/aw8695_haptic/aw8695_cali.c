...
#ifdef CONFIG_TTG_ACCESS_DIAG_PARTITION
...
static int aw8695_write_cali_lra_to_file(char cali_lra){
	struct file *fp;
	char buf[50] = {0};
	loff_t pos = 0;
	mm_segment_t fs;
...
static int aw8695_get_cali_lra_from_file(char *cali_lra){
	struct file *fp;
	struct inode *node;
	int f_size;
	char *buf;
...
#ifdef CONFIG_TTG_ACCESS_DIAG_PARTITION
	return aw8695_write_cali_lra_to_file(cali_lra);
#else
	return -EBUSY;
#endif
}
...
int aw8695_get_cali_lra_from_nvram(char *cali_lra){
	/*custom add, if success return value is 0 , else -1*/
#ifdef CONFIG_TTG_ACCESS_DIAG_PARTITION
	return aw8695_get_cali_lra_from_file(cali_lra);
#else
	return -EBUSY;
#endif
}
