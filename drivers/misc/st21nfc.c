...
// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_irq = devm_gpiod_get_optional(dev, "irq", GPIOD_IN);
...
// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_reset = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
...
// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_pidle = devm_gpiod_get_optional(dev, "pidle", GPIOD_IN);
	...
	st21nfc_dev->gpiod_clkreq = devm_gpiod_get_optional(dev, "clkreq", GPIOD_IN);
	...
	memset(nfc_version,0,sizeof(nfc_version));
     	snprintf(nfc_version, sizeof(nfc_version),"%s","st54jnfc");
