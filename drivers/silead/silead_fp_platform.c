...
int silfp_dev_init(void)
{
    ...
    LOG_MSG_DEBUG(ERR_LOG, "status: %d\n", status);
    if (status < 0) {
        return status;
    }
	...
	LOG_MSG_DEBUG(ERR_LOG, "1\n");
    status = platform_driver_register(&silfp_driver);
    LOG_MSG_DEBUG(ERR_LOG, "status a: %d\n", status);
    if (status < 0) {
