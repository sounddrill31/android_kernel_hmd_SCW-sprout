...
static void msm_restart_prepare(const char *cmd)
{
    ...
    pr_err("val_buf = %s!!!\n",val_buf);
	if(!strncmp(val_buf, "open", 4))
	{
	    force_warm_reboot = true;
		pr_err("force_warm_reboot %d!!!\n",force_warm_reboot);
	}

	if(!strncmp(val_buf, "dload", 5))
	{
	    download_mode = 1;
	...
	if (cmd != NULL) {
        ...
        // Add-begin by ning.wei for HMD custom reboot reason of imei corrupted
        if (!strncmp(cmd, "imei_corrupted", sizeof("imei_corrupted"))) {
            reason = PON_RESTART_REASON_IMEI_CORRUPTED;
        }
        // Add-end by ning.wei for HMD custom reboot reason of imei corrupted
