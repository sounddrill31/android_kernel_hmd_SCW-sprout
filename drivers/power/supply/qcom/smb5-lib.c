...
//merged by changxue.fang for thething charge,20210421,start
#ifdef CONFIG_HS_CHARGE_FG_FUNCTION
	if((chg->dam_type || chg->force_5v) && (val == QTI_POWER_SUPPLY_DP_DM_FORCE_9V ||
		val == QTI_POWER_SUPPLY_DP_DM_FORCE_12V)){
		val = QTI_POWER_SUPPLY_DP_DM_FORCE_5V;
		smblib_err(chg, "DAM TYPE ,FORCE 5V\n", val);
	}
				    ...
				    union power_supply_propval *val)
{
	...
	/* bug Z1S-4447,bin,20201106,add,begin */
	val->intval = MICRO_9V;
	/* bug Z1S-4447,bin,20201106,add,end */
...
//merged by changxue.fang for thething charge,20210421,start
#ifdef CONFIG_HS_CHARGE_FG_FUNCTION
       /*Solve DAM cable(Huawei rp-rp cable) can not charging, KBA-190702004817*/
       /*Config 0x154A to 0x17, PMI632 spec can't find this register, pls check PM7250B*/
       if (stat & (SNK_DAM_500MA_BIT | SNK_DAM_1500MA_BIT | SNK_DAM_3000MA_BIT)) {
		  chg->dam_type = true;
               ...
               smblib_err(chg,"%s: DAM cable found, will set usb typec source to defualt.\n", __func__);
       }
#else
	/* bug Z1S-5606,bin,20210222,add,begin */
	...
	/* bug Z1S-5606,bin,20210222,add,end */
#endif /*CONFIG_HS_CHARGE_FG_FUNCTION*/
//merged by changxue.fang for thething charge,20210421,end
	...
	/* bug Z1S-5606,bin,20210222,cancel,begin */
	// case SNK_DAM_500MA_BIT:
	// case SNK_DAM_1500MA_BIT:
	// case SNK_DAM_3000MA_BIT:
	//	return QTI_POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY;
	/* bug Z1S-5606,bin,20210222,cancel,end */
	default:
		break;
	}
	// Mod-end by ning.wei for DAM cable charging
					  ...
					  union power_supply_propval *val)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS)
		return smblib_get_charge_param(chg, &chg->param.dc_icl, &val->intval);
...
#ifdef CONFIG_HS_CHARGE_FG_FUNCTION
				rc = vote(chg->usb_icl_votable,
						SW_ICL_MAX_VOTER, true, chg->float_icl_ua);
				pr_err("%s set float current\n",__func__);
#else /*CONFIG_HS_CHARGE_FG_FUNCTION*/
...
////merged by changxue.fang for thething charge,20210421,start
	...
	present = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);

	pr_err("real_charger_type=%d,present=%d,chg->force_5v=%d\n",chg->real_charger_type,present,chg->force_5v);
	if (chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP
		&& present
		&& !strcmp(irq_data->name, "usbin-collapse")
		&& !chg->force_5v
		&& chg->mode == PARALLEL_MASTER) {
			...
			pr_err("hvdcp but vbus still at 5, force 5v\n");
	}
#endif /*CONFIG_HS_CHARGE_FG_FUNCTION*/
//merged by changxue.fang for thething charge,20210421,end
	smblib_dbg(chg, PR_OTHER, "IRQ: %s\n", irq_data->name);
	return IRQ_HANDLED;
}
...
#if IS_ENABLED (CONFIG_CHARGER_IDT_P9415)
		//remove dc limit
		vote(chg->fcc_votable, DC_LIMIT_VOTER, false, 0);
#endif /*CONFIG_CHARGER_IDT_P9415*/
...
#ifdef CONFIG_TOUCHSCREEN_USB_CHECK
	usb_flag = vbus_rising;

	pr_err("%s:Touchsreen usb check usb_flag = %d\n", __func__, usb_flag);
	schedule_work(&chg->ctp_work);
#endif /* CONFIG_TOUCHSCREEN_USB_CHECK */
	power_supply_changed(chg->usb_psy);
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: usbin-plugin %s\n",
					vbus_rising ? "attached" : "detached");
...
#ifdef CONFIG_TOUCHSCREEN_FT_SPI_FH
extern int ctp_work_with_ac_usb_plugin_ftp(int plugin);
#endif /* CONFIG_TOUCHSCREEN_FT_SPI_FH */

static void smb_ctp_work(struct work_struct *work)
{
#ifdef CONFIG_TOUCHSCREEN_FT_SPI_FH
	ctp_work_with_ac_usb_plugin_ftp((usb_flag ? 1 : 0));
#endif /* CONFIG_TOUCHSCREEN_FT_SPI_FH */
}
...
static void smblib_handle_slow_plugin_timeout(struct smb_charger *chg,
					      bool rising)
{
	smblib_dbg(chg, PR_OTHER, "IRQ: slow-plugin-timeout %s\n",
...
static void smblib_handle_sdp_enumeration_done(struct smb_charger *chg,
					       bool rising)
{
	smblib_dbg(chg, PR_OTHER, "IRQ: sdp-enumeration-done %s\n",
		   rising ? "rising" : "falling");
}
					      ...
					      bool rising)
{
	...
	smblib_dbg(chg, PR_OTHER, "IRQ: hvdcp-3p0-auth-done rising; %s detected\n",
		   apsd_result->name);
}
					      ...
					      bool rising, bool qc_charger)
{
	...
	smblib_dbg(chg, PR_OTHER, "IRQ: %s %s\n", __func__,
		   rising ? "rising" : "falling");
}
...
static void smblib_handle_hvdcp_detect_done(struct smb_charger *chg,
					    bool rising)
{
	smblib_dbg(chg, PR_OTHER, "IRQ: hvdcp-detect-done %s\n",
		   rising ? "rising" : "falling");
}
...
irqreturn_t smb5_usb_source_change_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_OTHER, "APSD_STATUS = 0x%02x\n", stat);
...
irqreturn_t smb5_dc_plugin_irq_handler(int irq, void *data)
{
...
#if IS_ENABLED (CONFIG_CHARGER_IDT_P9415)
	/*if usb is present,dc not work*/
	if(vbus_present && dcin_present){
		smblib_rerun_apsd(chg);
		smblib_dbg(chg, (PR_WLS | PR_INTERRUPT), "usb is present,dc return\n");
		return IRQ_HANDLED;
	}
#endif /*CONFIG_CHARGER_IDT_P9415*/
	...
	if (dcin_present && !vbus_present) {
		...
		//vote(chg->fcc_votable, DC_LIMIT_VOTER, true , 2000000);
		if (chg->mode == PARALLEL_MASTER) {
			vote(chg->pl_enable_votable_indirect, USBIN_V_VOTER, true, 0);
		}
	...
	smblib_dbg(chg, (PR_WLS | PR_INTERRUPT), "dcin_present= %d, usbin_present= %d, cp_reason = %d,"
			"real_charger_type=%d\n",dcin_present, vbus_present, chg->cp_reason,chg->real_charger_type);
...
static void jeita_update_work(struct work_struct *work)
{
	...
	smblib_err(chg, "D85 Version, use ontim,jeita-hard-thresholds prop.\n");
...
#ifdef CONFIG_TOUCHSCREEN_USB_CHECK
	INIT_WORK(&chg->ctp_work, smb_ctp_work);
#endif /* CONFIG_TOUCHSCREEN_USB_CHECK */
