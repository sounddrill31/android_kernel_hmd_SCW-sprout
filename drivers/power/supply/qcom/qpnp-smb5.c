...
#include <linux/thermal.h> // ning.wei++
#include "smb5-reg.h"
#include "smb5-lib.h"
#include "smb5-iio.h"
...
static int smb5_parse_dt_currents(struct smb5 *chip, struct device_node *node)
{
	...
	if (chip->dt.usb_icl_ua > 0)
		chg->hw_max_icl_ua = chip->dt.usb_icl_ua;
...
static enum power_supply_property smb5_usb_props[] = {
	...
	POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE, // ning.wei++
	POWER_SUPPLY_PROP_TEMP,	// ning.wei++ for quiet temperature
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		...
		val->intval = get_client_vote(chg->usb_icl_votable,
					      USB_PSY_VOTER);
		break;
	// Add-begin by ning.wei for usb real apsd type, usb voltage , usb current
	...
	case POWER_SUPPLY_PROP_TEMP:
			{
				...
				thermal_dev = thermal_zone_get_zone_by_name("quiet-therm-usr");
				if (!IS_ERR_OR_NULL(thermal_dev)) {
					thermal_zone_get_temp(thermal_dev, &quiet_temp);
				}
				val->intval = quiet_temp/100;
			}
			break;
	// Add-end by ning.wei for usb real apsd type, usb voltage , usb current
	default:
...
static enum power_supply_property smb5_batt_props[] = {
	...
	//POWER_SUPPLY_PROP_TIME_TO_FULL_NOW, // ning.wei-- remove qcom one, battery info will use google algorithm: CT-9659
};
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		pr_err("D85 version, temp: %d.\n", pval->intval);
		if (pval->intval > 450) // 45C
			pval->intval= 450;
		else if (pval->intval < 100) // 10C
			pval->intval = 100;
#endif
		break;
...
#ifdef CONFIG_HS_BATTERY_THERM_NTC_10K
	/*Set battery thermal internal pull up to 30k, solve battery overheat issue.*/
	rc = smb5_configure_internal_pull(chg, BAT_THERM, INTERNAL_PULL_30K_PULL);
#endif /*CONFIG_HS_BATTERY_THERM_NTC_10K*/
	return rc;
}
...
#ifdef SMT_VERSION
static int typec_cc_orientation_show(void *data, u64 *val)
{
	struct smb_charger *chg = data;
	int orientation;
	int ret = 0;
	...
	pr_info("%s:\n", __func__);
	ret = smblib_get_prop_typec_cc_orientation(chg, &orientation);
	if (ret < 0) {
		pr_err("%s: read cc orientation error: %d\n", __func__, ret);
		return -ENODATA;;
	}

	*val = orientation;
	pr_info("%s: cc orientation: %d\n", __func__, orientation);

	return 0;
}

DEFINE_DEBUGFS_ATTRIBUTE(typec_cc_orientation_fops,
			typec_cc_orientation_show, NULL, "%lld\n");
#endif
...
#ifdef SMT_VERSION
	file = debugfs_create_file("typec_cc_orientation", 0644,
				chip->dfs_root, chip, &typec_cc_orientation_fops);
	if (IS_ERR_OR_NULL(file))
		pr_err("Couldn't create typec_cc_orientation file rc=%ld\n",
			(long)file);
#endif
}
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	// req,bin,20201106,add,begin
	if (chg->chg_param.smb_version == PM7250B) {
		snprintf(pmic_version, sizeof(pmic_version),"%s", "PM7250B");
	}	
	// req,bin,20201106,add,end
