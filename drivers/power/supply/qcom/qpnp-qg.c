...
char bat_name[32]="unknown";
EXPORT_SYMBOL(bat_name);
...
static int qg_remove_device(struct qpnp_qg *chip)
{
	pr_info("unregister qg soc devices\n");
...
static int get_batt_id_ohm(struct qpnp_qg *chip, u32 *batt_id_ohm)
{
	...
	*batt_id_ohm = div64_u64(BID_RPULL_OHM * 1000 + denom / 2, denom);
	...
	pr_err("batt_id_mv=%d, batt_id_ohm=%d\n", batt_id_mv, *batt_id_ohm); // ning.wei++ log
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	memset(bat_name, 0, sizeof(bat_name));
	memcpy(bat_name, chip->bp.batt_type_str, 
		strlen(chip->bp.batt_type_str) > (sizeof(bat_name) + 1) ? sizeof(bat_name) : strlen(chip->bp.batt_type_str));
...
#define QG_TTF_ITERM_DELTA_MA		100
