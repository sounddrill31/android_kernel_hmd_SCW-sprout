			 ...
			 int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		ret = gi2c->err;
		if (gi2c->err) {
		    // Add-begin by ning.wei for show i2c address when transfer error
		    #if 0
			GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
				"i2c error :%d\n", gi2c->err);
		    #else
			GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
				"%d-00%02x i2c error :%d\n", adap->nr, msgs[0].addr, gi2c->err);
		    #endif
		    // Add-end by ning.wei for show i2c address when transfer error
			break;
		}
	}
geni_i2c_txn_ret:
