...
//merged by changxue.fang for thething sdcard_status,20210406,start
#if IS_ENABLED(CONFIG_TTG_BOOT_INFO)
#include <linux/of_gpio.h>
#include <linux/his_debug_base.h>
#endif /*CONFIG_TTG_BOOT_INFO */
//merged by changxue.fang for thething sdcard_status,20210406,end
...
/*add start: add device node for getting status of TFCard holder in factory mode */
#if IS_ENABLED(CONFIG_TTG_BOOT_INFO)
static ssize_t sdcard_status_show(struct kobject *kobj,
...
static struct kobj_attribute sdcard_status_attribute =
	__ATTR(sdcard_status, S_IRUGO, sdcard_status_show, NULL);
#endif /*CONFIG_TTG_BOOT_INFO */
/*add end*/
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
...
#endif
	msm_host->mmc->caps2 |= MMC_CAP2_MAX_DISCARD_SIZE;//ling.yi add for QKS-1405 format some sdcard too slowly.
	...
	if(!strcmp(mmc_hostname(host->mmc), "mmc1")) {
		if ( his_register_sysfs_attr(&sdcard_status_attribute.attr) < 0) {
