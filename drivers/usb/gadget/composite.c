		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	int			tmp;
//zhongli.liu porting for thething bring up 20210201 start
#ifdef BUILD_THETHING_PROJECT
       /* add for solving usb device repeating enumeration thething bring up add begin*/
       pr_info("%s enter!\n", __func__);
       ...
       /* add for solving usb device repeating enumeration thething bring up add end*/
#endif       
//zhongli.liu porting for thething bring up 20210201 end
	if (number) {
		list_for_each_entry(c, &cdev->configs, list) {
