...
static int st54j_se_probe(struct spi_device *spi)
{
	...
	dev_info(dev, "%s: eSE is configured\n", __func__);
...
static int __init st54j_ese_init(void)
{
    int ret = 0;
    pr_info("Loading st54j_ese driver\n");
    ret = spi_register_driver(&st54j_se_driver);
    if ( ret != 0 ) {
       pr_info("Loading st54j_ese driver error\n");
    }
    return ret;
}

static void __exit st54j_ese_exit(void)
{
	pr_info("Unloading st54j_ese driver\n");
    spi_unregister_driver(&st54j_se_driver);
}
//module_spi_driver(st54j_se_driver);
module_init(st54j_ese_init);
module_exit(st54j_ese_exit);
