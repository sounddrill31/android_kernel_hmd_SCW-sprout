...
static int aw882xx_monitor_get_data(struct aw882xx *aw882xx)
{
	...
	aw_dev_dbg(aw882xx->dev, "%s: vol: ipeak = 0x%x, gain = 0x%x, vmax = 0x%x\n",
				...
				uint16_t gain)
{
	...
	if (read_volume == set_volume) {
		aw_dev_dbg(aw882xx->dev, "%s: set db = %d.%d dB, no change\n",
	...
	aw_dev_dbg(aw882xx->dev, "%s: set_volume = %d.%d dB\n",
...
void aw882xx_monitor_stop(struct aw882xx_monitor *monitor)
{
	...
	aw_dev_info(aw882xx->dev, "%s: enter\n", __func__);
	if (delayed_work_pending(&aw882xx->monitor.delay_work))
					...
					const struct firmware *cont)
{
	...
	aw_dev_dbg(aw882xx->dev, "%s: chip name:%s\n",
		__func__, monitor_hdr->chip_type);
	aw_dev_dbg(aw882xx->dev, "%s: ui ver:0x%x\n",
					...
					void *context)
{
	...
	if (aw882xx->monitor.monitor_cfg->monitor_status == AW_MON_CFG_ST) {
		...
		ret = aw_parse_monitor_profile(aw882xx, cont);
		if (ret < 0)
