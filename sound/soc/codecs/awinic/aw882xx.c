...
{
	return (((value / AW882XX_VOL_6DB_STEP) << 4) + (value % AW882XX_VOL_6DB_STEP));
}
...
static int aw882xx_probe(aw_snd_soc_codec_t *codec)
{
	...
	aw882xx->codec = codec;
	schedule_delayed_work(&aw882xx->delay_work, msecs_to_jiffies(8000));
...
void aw882xx_delay_load_work(struct work_struct *work)
{
	struct delayed_work *p_delayed_work = container_of(work, struct delayed_work, work);
	struct aw882xx *aw882xx = container_of(p_delayed_work,struct aw882xx, delay_work);
	aw_dev_info(aw882xx->dev, "%s enter\n", __func__);
	aw882xx_load_monitor_profile(&aw882xx->monitor);
}
	...
	const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&aw882xx->delay_work, aw882xx_delay_load_work);
